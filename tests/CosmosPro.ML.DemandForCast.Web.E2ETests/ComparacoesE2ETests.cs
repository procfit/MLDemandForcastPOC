...
public sealed class ComparacoesE2ETests(AppHostFixture fixture)
{
    ...
    public async Task Cria_sessao_e_ve_instrucoes_de_extracao()
    {
        ...
        // NÃO se espera pela URL aqui, e a ausência é deliberada. `WaitForURLAsync` aguarda o
        // estado "Load", que a navegação client-side do Blazor não dispara — ela estourava por
        // timeout de forma intermitente, sob carga, com a página de destino já na tela. E o
        // wait era redundante desde sempre: o locator abaixo casa com um elemento que SÓ existe
        // no destino, então esperá-lo já é a prova de que a navegação aconteceu.
        //
        ...
        // 60s como o resto da suíte: o clique dispara CreateAsync (round-trip SignalR + HTTP)
        // antes de navegar, e 15s ficaram apertados quando a suíte cresceu.
