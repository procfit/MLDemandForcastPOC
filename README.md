...
| Função de perda do LightGBM (`objective`) | **Não exposta.** Nem `LightGbmRegressionTrainer.Options` nem a `OptionsBase` herdada têm a chave — verificado na superfície pública do pacote 4.0.2 (zero ocorrências de `objective`, `tweedie` ou `poisson`) e confirmado via Context7. O objetivo fica fixo em erro quadrático; o `Poisson` do ML.NET é um trainer **linear** separado (`LbfgsPoissonRegression`), não um objetivo da árvore. | Fecha a resposta usual para excesso de zeros (Tweedie/Poisson), que é o regime do dado real (86,6% dos dias-item sem venda). O contorno que cabe dentro do ML.NET é o **modelo em dois estágios** (`HurdleForecastEngine`) — medido, e com ganho em todas as métricas primárias; ver F17 no roadmap. |
...
| `images` | `ubuntu-latest` | Só se os dois anteriores passarem: constrói e empurra a **imagem base do worker** (abaixo), `aspire do push` (constrói e empurra as quatro imagens, na tag imutável), dois **smokes** que abrem as imagens publicadas — o do worker confere as dependências nativas do LightGBM, o da Web confere que o extrator foi de fato para dentro dela e na versão deste run —, um passo de `docker tag`/`docker push` que acrescenta a tag móvel, `aspire publish` (gera `docker-compose.yaml` + `.env`, publicados como artefato `aspire-compose`) e a **conferência do compose**: o gerado é comparado com `deploy/docker-compose.yaml` e a divergência é build vermelho (ver "O que o operador precisa preencher no `.env`"). |
...
- [x] **F14 — Sessões de comparação**: fluxo guiado de ponta a ponta para o comprador da rede. Ele cria uma **sessão** no painel `/`, baixa o extrator, escolhe **uma** sugestão de compra no PBS, sobe o ZIP, e a sessão anda sozinha — importar → treinar → comparar → concluir — até uma tela (`/comparacoes/{id}`) que diz, em linguagem de negócio, quanto capital cada método deixaria parado na prateleira. Plano em [Docs/superpowers/plans/2026-07-28-f14-sessoes-de-comparacao.md](Docs/superpowers/plans/2026-07-28-f14-sessoes-de-comparacao.md), spec em [Docs/superpowers/specs/2026-07-28-sessoes-de-comparacao-design.md](Docs/superpowers/specs/2026-07-28-sessoes-de-comparacao-design.md). As **Tasks 3–4 do plano da F12** foram substituídas por esta fase — a UX do extrator passou a ser "uma sugestão, janelas derivadas".
  ...
  - **Camada analítica da tela de itens** (pedido do comprador, itens 9 e 10 do documento de 25/08): filtros **combináveis** por Loja, Categoria e Curva do ERP; **totalizadores do recorte** (itens, compra PBS/ML, sobra PBS/ML, diferença, R$ parado de cada braço) recalculados no servidor a cada filtro; e **exportação para Excel** respeitando o filtro, com aba de capa declarando o recorte. Filtro, contagem, totais e planilha passam pela **mesma cláusula** (`ComparacoesEndpoints.AplicarFiltros`) — se divergissem, o comprador veria 31 itens na tela e um total apurado sobre 20.153. `Categoria` é gravada na materialização (migration `AddCategoriaNoItemDaSessao`, sem backfill: a categoria vive no Stage, que cada import substitui, e preencher retroativamente atribuiria o cadastro do envio atual a uma sessão que descreve outro). **`Departamento` não existe** — nem no contrato de extração do PBS nem em `Stage.Produtos` —, e a tela diz isso no tooltip em vez de inventá-lo a partir da categoria. **Armadilha registrada:** `SUM` de `decimal?` traduzido pelo EF Core devolve **0**, não nulo, quando nenhuma linha tem valor; as somas do braço de ML são decididas pela contagem de linhas que contribuíram, senão "sem cálculo" viraria "o ML mandaria não comprar nada".
  - **Download do ZIP enviado** (`GET /api/comparacoes/{id}/dados`, mesma rota do POST no verbo oposto; botão no cabeçalho de `/comparacoes/{id}`): devolve o arquivo **byte a byte** como veio. Existe para repetir uma comparação sobre exatamente o mesmo envio — cada import substitui o Stage inteiro da rede, então o dado que uma sessão antiga descreve já não está lá, e pedir uma extração nova produziria outro arquivo, de outro instante, com outra janela de vendas: duas execuções assim não se comparam. O ZIP sobrevive à exclusão da sessão de propósito. **404 cobre três casos** — sessão inexistente, de outra rede, ou sem envio —, porque 403 no segundo confirmaria a quem sondasse que a sessão existe em outro inquilino. O salto pela Web repassa o stream sem materializar o arquivo, já que a apiservice não tem endpoint externo.
  - **`SessaoWorker`**: um `BackgroundService` novo com o mesmo claim `UPDLOCK/READPAST` das outras filas, que a cada volta observa o job da fase corrente e, quando ele conclui, cria o da fase seguinte. `ImportWorker`, `TreinoWorker` e `ComparacaoWorker` seguem **sem saber que sessões existem** — a ordem das fases mora num único lugar. A decisão de avanço é pura (`SessaoAvancador.ProximoEstado`) e os dois parâmetros derivados dos jobs também (`SessaoJobs.Treino`/`Comparacao`), porque errar neles não dá erro: produz um treino que roda liso e uma comparação recusada três fases depois. Corte de treino = **o próprio dia da sugestão** (o loader aplica `Data < @treinoAte`); orçamento de SKUs = **nenhum** (`MaxSkus = null`): o treino carrega o catálogo inteiro da rede. Houve um `Math.Clamp(skusDistintos, 80, 1000)` aqui, e o teto vinha do limite de 2100 parâmetros por comando do SQL Server, que o `Sku IN (@s0…@sN)` do loader gastava um por SKU. `EscopoDeSkus` trocou o `IN` por join com tabela temporária, que não tem teto; o parâmetro continua existindo (nulo = todos) para experimento e teste. **Medido depois, e menor do que se supunha:** na sugestão 125595 da Retiro só **991** SKUs tinham venda antes do corte, contra um teto de mil — ele não excluía SKU nenhum. Remover o teto deixou a cobertura **idêntica** (147 itens na decisão, 563 na taxa) e mexeu pouco no viés (MAE 0,47 → 0,44); o treino levou ~40s nos dois casos. O ganho valeu por outro motivo: fechou um bug latente em `StageEstoqueInicialLoader`, que estouraria numa simulação com mais de 2100 SKUs, e tirou um teto arbitrário para catálogos maiores. Ver a nota de `ItensForaOrcamentoSkus` para o que os 54% de itens fora da população realmente são.
  ...
  - **A materialização não conclui mais a sessão** — desde a F15 ela grava `AguardandoAvaliacao`, e a última transição passou a ser feita pelo endpoint da Seção G (`POST /api/comparacoes/{id}/avaliacao`; até 19/09/2026 era o envio do questionário). O resto do parágrafo acima continua valendo palavra por palavra: mesma transação, mesmo `WHERE Status = <fase reclamada>`.
...
- [ ] **F15 — Avaliação da execução e questionário do comprador**: o dado de campo do TCC. **São duas coisas diferentes**, e o desenho mudou em 19/09/2026 por orientação do Professor, registrada pelo patrocinador no documento de 16/09.
  - **A Seção G é por execução e é o que a conclui**: `Comparando → AguardandoAvaliacao → Concluida`, e `Concluida` significa "o comprador registrou o veredito desta execução". **O questionário é por COMPRADOR**, respondido uma vez só, liberado depois de `QuestionarioCatalogo.MinimoDeExecucoes` (2) execuções avaliadas por ele.
  - **Por que o desenho anterior caiu.** Até aqui o envio do questionário é que concluía a sessão — decisão do próprio patrocinador (resposta 4a), que ele reverteu. Com o questionário respondido uma vez só, aquele desenho travaria tudo: da terceira execução em diante nenhuma sessão sairia de `AguardandoAvaliacao`, que é a fase que nenhum worker reclama.
  - **A única transição da máquina de estados que não sai do Worker.** `AguardandoAvaliacao` está fora do allowlist do `SessaoWorker.ClaimNextAsync`, então nenhuma fila a reclama e o relógio de `LimiteDeFaseSemProgresso` não a alcança — o que é o comportamento certo, porque o que falta é um humano. Quem grava `Concluida` é `POST /api/comparacoes/{id}/avaliacao`. Registrar de novo é **correção do veredito**, não transição: a guarda olha o status de origem, e um `PodeTransicionar` cego recusaria `Concluida → Concluida`.
  - **`PodeExcluir` tem uma segunda recusa, por motivo diferente da primeira.** As três fases de worker recusam para proteger o *job*; `Concluida` recusa para proteger o *dado* — avaliação de pesquisa não evapora por clique. A equivalência que ela exige é "concluída ⟺ **avaliada**", garantida pela migration `RenomeiaStatusParaAguardandoAvaliacao`, que devolve a `AguardandoAvaliacao` as sessões concluídas por envio de questionário sem veredito. É o espelho do que a `AddQuestionarios` fez quando a equivalência anterior nasceu.
  - **`Questionarios` é chaveada por `UsuarioId`** (`UQ_Questionarios_UsuarioId`, migration `QuestionarioPorComprador`). Saíram `SessaoId`, a FK `Cascade` para a sessão — o veredito sobre a *ferramenta* não pode sumir quando uma execução é excluída — e `ItensComDecisaoMl`/`TotalDeItens`, que descreviam *a* execução avaliada e passariam a descrever uma arbitrária entre as 2+ cobertas. `QuestionarioRespostas` mantém PK `(QuestionarioId, PerguntaCodigo)`. O cabeçalho **não tem coluna de situação**: `EnviadoEm` nulo é rascunho, preenchido é selado.
  - **O portão é do servidor.** `GET /api/questionario` devolve `ExecucoesAvaliadas`, `MinimoExigido` e `Liberado`; `PUT`/`POST` recusam com 409 antes da liberação e depois do envio. Conta execução com Seção G **respondida por aquele comprador** — execução sem veredito não conta, e avaliação de outra pessoa não abre o portão de ninguém. A tela explica quanto falta em vez de esconder o botão.
  - **A versão do instrumento saiu do código e da UI.** "V3"/"V5"/"V6"/"V7" é controle do documento de quem conduz a pesquisa; o usuário final não vê versão em lugar nenhum, e `VersaoCatalogo` é carimbo silencioso sem leitor. O mapa `NomeDaVersao` que traduzia o contador no nome **foi removido** em 21/09/2026; a lista de qual contador é qual documento vive no doc-comment de `QuestionarioCatalogo.Versao`. O que mantém a resposta legível é o retrato do enunciado gravado com ela. Contrato em [CLAUDE.md §4](CLAUDE.md).
  - **O instrumento é o Apêndice A da dissertação** (MGSI, ISCTE-IUL). Hoje é o **Questionário V7**, `Versao = 5`: **Parte A** com A1 (função), A2 (anos na área, ordinal) e A3 (a organização já usa sistema de previsão); **Parte B** com **onze** afirmações (B1–B11) em escala de Likert 1–5. Texto **literal, em português europeu** — "artefacto", "stocks", "ruturas" —, deliberadamente não traduzido para o pt-BR do resto da aplicação: reescrever enunciado invalida a comparação com o que foi submetido. A apresentação e o termo de consentimento aparecem **fora do wizard**, acima do primeiro passo, porque é o que o participante concorda antes de responder.
  - **Histórico do instrumento, porque ele muda a leitura dos dados.** V3 (`Versao = 2`) tinha sete afirmações B1–B7. A V5 (`3`) foi para doze, B1–B12, **reaproveitando o código B7**. A V6 (`4`) removeu a antiga B6 — a única que perguntava sobre coisas que o artefacto não faz — e renumerou as seguintes, deixando onze. A **V7 (`5`, 21/09/2026) reescreveu B3, B10 e B11 e NÃO remanejou nada**, sendo a primeira revisão que não cria armadilha de agregação. Onde o código mudou de dono, agrupe por `(VersaoCatalogo, Codigo)` e nunca por código solto; a afirmação de intenção de uso, por exemplo, foi B7 na V2, B12 na V5 e é B11 desde a V6. **B11 mudou de sentido na V7**, não só de redação: de potencial institucional para intenção de uso em primeira pessoa.
  - **O consentimento promete confidencialidade, não anonimato — e isso é deliberado.** O .docx afirma que a participação é "anónima" e que nada permite identificar os participantes; o sistema grava `Questionarios.UsuarioId`, e desde 19/09/2026 esse vínculo é a **chave única** da tabela (`UQ_Questionarios_UsuarioId`), porque a regra "um questionário por comprador" depende dele. A frase de anonimato já voltou três vezes no documento e nunca foi restaurada no código; há teste que falha se ela reaparecer. A divergência entre o Apêndice A anexado ao trabalho e o texto que o participante lê na tela é decisão de quem conduz a pesquisa.
  ...
  - **Rascunho e envio são coisas diferentes.** `PUT /api/questionario` grava parcial (é o que permite fechar o navegador no meio sem perder o preenchido) e substitui o conjunto inteiro — a tela manda o estado completo do wizard, então pergunta ausente do corpo significa "sem resposta", não "mantenha a antiga". `POST .../enviar` exige completude e sela `EnviadoEm`. **Não move sessão nenhuma** desde 19/09/2026 — quem conclui a execução é a Seção G, e o questionário não pertence a execução alguma; a corrida que sobra é entre dois envios do mesmo comprador, resolvida pelo índice único `UQ_Questionarios_UsuarioId`. Depois de selado é imutável.
...
- [x] **F16 — Dados de mercado IQVIA por rede (partes A e B)**: o dado de mercado saiu do ZIP de import e ganhou ciclo de vida próprio. Usando o software ficou claro que o desenho da F2 estava errado: `mercado_iqvia.csv` viajava no mesmo ZIP das vendas, o extrator o gerava sempre **vazio** (o ERP não tem esse dado), ninguém o consumia, e cada import fazia `DELETE ... WHERE RedeId` — o dado de mercado morreria a cada envio de sessão. O formato real também é outro: o relatório mensal da IQVIA é um **XLSX cross-tab por EAN × brick × bandeira × mês** (~52 mil linhas × 24 colunas de medida, chave EAN e não princípio ativo, geografia por brick e não UF, com a dimensão própria × concorrentes que não existia no schema antigo).
  - **Removido**: `Stage.dbo.MercadoIqvia`, `mercado_iqvia.csv` do contrato do ZIP (ZIPs antigos seguem válidos — entrada desconhecida é ignorada), a geração no extrator e no sintético. O deploy do DACPAC ganhou `DropObjectsNotInSource` para a tabela sumir dos bancos já provisionados.
  - **Adicionado**: 4 tabelas no `engine` (migration `AddMercadoIqvia`) — `MercadoCargas` (fila), `MercadoObservacoes` (forma longa, PK `(RedeId, Mes, Brick, Bandeira, Ean)`, substituição por `(mês, brick)`), `MercadoProdutos` (dimensão por EAN) e `MercadoBrickPdvs` (brick → CNPJs do painel, a futura ponte com as lojas); `Stage.Lojas.Cnpj` + extração de `ENTIDADES.CGC` no PBS; parser streaming `IqviaXlsxParser` (sem pacote novo, ~2,4 s no arquivo real de 9,6 MB); endpoints `/api/mercado/*`; tela **Dados de mercado** (`/mercado`) com upload, cobertura mês × brick e envios com polling. Invariantes (zero ≠ não-coberto, semântica do `ValorCpp` normalizado, cabeçalho como contrato) em [CLAUDE.md §4](CLAUDE.md).
  - **Fora deste corte (parte C, próxima)**: join EAN↔`Produtos.Ean` e CNPJ↔brick, features de mercado no `FeatureBuilder` (com corte anti-vazamento sobre `Mes` — o agregado mensal contém o dia-alvo), indicação "usou IQVIA" na sessão (cobertura percentual materializada no treino, não booleano) e backtest walk-forward contra o baseline sem mercado.
- [x] **F16 parte C, grupo B — Alertas de mercado na tabela de decisão**: o dado de mercado passou a **explicar itens ao comprador**, ao lado da decisão PBS × ML. Regras B1, B2, B3 e B6 do documento de controle (aba `REGRA DE NEGÓCIO IQVIA`), que são os itens 9 e 10 do acompanhamento — os dois marcados como críticos. Spec em [Docs/superpowers/specs/2026-08-30-iqvia-alertas-ao-comprador-design.md](Docs/superpowers/specs/2026-08-30-iqvia-alertas-ao-comprador-design.md), plano em [Docs/superpowers/plans/2026-08-30-f16c-alertas-de-mercado-grupo-b.md](Docs/superpowers/plans/2026-08-30-f16c-alertas-de-mercado-grupo-b.md).
  - **A leitura antiga da parte C virou o "Caminho 2", e ficou para depois.** Ela era "IQVIA vira covariável no `FeatureBuilder`"; o pedido do documento de controle é camada de alertas, o ganho da covariável é incerto por descasamento de grão (IQVIA é mensal por brick, a previsão é diária por loja), e o grupo B constrói as duas pontes que a covariável precisaria de qualquer forma.
  - **Sete colunas em `ComparacaoSessaoItens`** (migration `AddMercadoNoItemDaSessao`): mês usado, brick, unidades da rede e dos concorrentes, índice de desempenho, dias sem estoque e alerta. **Materializadas**, e não consultadas na abertura da tela, pelo mesmo motivo do resto do resultado — o Stage é apagado a cada import. Todas anuláveis, e **nulo é "não foi possível calcular", nunca zero**: zero é medição da IQVIA, e zero nosso com o bairro vendendo é o alerta mais forte que existe.
  - **A régua do B2 não é fatia de mercado** — é a fatia do item dividida pela fatia agregada da rede no mesmo brick. O tamanho da rede está nos dois lados e cancela, então o que sobra é o efeito do produto. A régua natural (`fatia real ÷ fatia esperada pelo número de PDVs`) **não roda**: o contador de PDVs concorrentes vive numa área de tabela dinâmica que o `IqviaXlsxParser` ignora, então `MercadoBrickPdvs` guarda uma linha de concorrentes, não 37. Medido em junho/2026: a régua adotada dispara em 15,0%, a outra em 8,5%.
  - **O mês comparado é o último coberto estritamente anterior ao mês da sugestão.** Uma regra, sem caso especial: com um relatório só carregado ela cai no espelho do ano anterior (sazonalmente casado); com a série empilhada, no mês imediatamente anterior. O "estritamente" sustenta a afirmação da dissertação — o mês da sugestão contém as consequências dela, e comparar contra ele seria circular.
  - **B4 e B5 estão diferidas por impossibilidade no dado, não por prioridade.** `Real CPP ÷ Unidades` é idêntico entre bandeiras no mesmo brick e mês (37.410 pares medidos, zero diferença) e idêntico entre bricks (55.979 pares): é preço de referência normalizado, e **o relatório não contém preço praticado por concorrente**. B4 devolveria sempre um número que não significa o que a regra promete; B5 existia só para alimentá-la.
  - **A normalização do EAN foi o achado que salvou a fase.** O PBS grava 14 caracteres com zero à esquerda (`07896094928060`) e a IQVIA grava 13 (`7891721201806`): comparação exata casa **zero**. Sem medir isso antes de codar, o loader devolveria dicionário vazio em toda sessão, sem erro e sem log. Medido contra o cadastro real: 60,2% dos 28.987 produtos com EAN casam pela regra "sem zeros à esquerda"; na população de uma sugestão real, 21 de 43 SKUs (48,8%). EAN que a IQVIA não reportou **não é falha de join** — a lista dela é "o que teve movimento nos bricks pedidos", não um catálogo.
  - **Falta um extrator novo chegar ao comprador**, e a tela diz isso quando acontece. Os ZIPs de hoje não trazem `Cnpj` em `lojas.csv`, então nenhuma loja se liga a brick nenhum. O sinal testado é a **capacidade** (contagem de lojas com CNPJ), não a versão declarada no manifesto — o `0.17.0` do repositório emite `Cnpj` e o `0.17.0` que o comprador usou não emite, então número de versão mente aqui.
  - **Pendência que não é de código:** `FARMA ONE` tem 6 PDVs próprios no painel da IQVIA e **nenhuma coluna de venda**. As vendas deles estão dentro de `CONCORRENTES` ou de `DROGARIA RETIRO`, e o arquivo não diz qual — se estiverem em `CONCORRENTES`, todo alerta enviesa para disparar. Pergunta para quem puxa o relatório; o parser já trata qualquer bandeira diferente de `CONCORRENTES` como própria.
  - **Grupo A veio a seguir**, no mesmo corte de entrega — ver o item abaixo.
- [x] **F16 parte C, grupo A — Oportunidades de sortimento**: a tela `/mercado/oportunidades` lista o que os concorrentes vendem nos bairros da rede e **não existe no cadastro dela**. Regras A1, A2 e A3 do documento de controle. Plano em [Docs/superpowers/plans/2026-08-31-f16c-oportunidades-de-sortimento-grupo-a.md](Docs/superpowers/plans/2026-08-31-f16c-oportunidades-de-sortimento-grupo-a.md).
  - **O portão veio antes do desenho, e mediu o risco de a tela mentir.** A1 afirma "o mercado vende isto e você não tem" comparando código de barras; produto que a rede **tem** sob cadastro sem código fica invisível e viraria falso positivo — erro na direção que faz o comprador comprar o que já tem. Medido na instância da Natusfarma em 2026-08-31: dos **79.873** registros do mestre, **47.658 estão inativos** e quase nenhum tem EAN; sobre o cadastro **ativo** a cobertura é de **90,2%**, e **87% da lacuna** (2.756 de 3.162) está na seção `NÃO DEFINIDO`, que é registro incompleto. Nas seções que a IQVIA cobre — medicamento, MIP, perfumaria, mercearia — a cobertura vai de **98,6% a 99,4%**, e a lacuna é de 362 produtos em ~28.800: **1,3%**. Isso põe o falso positivo na faixa "abaixo de 10%" da regra de decisão, então a mitigação é só a declaração na tela — saiu a coluna de casamento por nome, que com 99% mediria ruído. **A leitura anterior, de "63,6% sem código", estava contando cadastro morto.**
  - **`RedeCatalogoEans` no `engine`, nunca no Stage** (migration `AddRedeCatalogoEans`, PK `(RedeId, Ean)`). A tela não pertence a sessão nenhuma e cada import apaga o Stage da rede — no Stage o catálogo zeraria no envio seguinte e a tela passaria a oferecer o cadastro inteiro como oportunidade, que é o defeito que a F16 corrigiu ao remover `MercadoIqvia`. **Substituição por rede inteira** a cada envio: é retrato do cadastro, e produto descadastrado tem de sair.
  - **`catalogo_eans.csv` no extrator, sem escopo por sugestão** — o único CSV do ZIP que ignora o escopo por SKU de propósito, e o único **sem tabela no Stage**. Contra um cadastro escopado, todo produto de fora pareceria ausente e a lista viraria itens que a rede já vende; há teste afirmando a ausência dele no mapeamento CSV→tabela, para ela não ser "corrigida". Sai com ~29 mil linhas (o filtro de EAN utilizável descarta os dois terços de cadastro morto). **Extrator 0.17.0 → 0.18.0**, e o bump não é formalidade: o `0.17.0` do repositório emite `Cnpj` e o `0.17.0` que o comprador usava não emitia.
  - **Três recusas que não são estilo.** Catálogo vazio devolve lista **vazia**, e não o mercado inteiro (catálogo vazio significa "não enviaram o arquivo", nunca "a rede não tem produto"). ZIP **sem** o catálogo não apaga o que existia (apagar puniria o comprador por usar um build velho). E só a venda de `CONCORRENTES` conta — somar a bandeira própria faria a tela oferecer o próprio sortimento de volta.
  - **A tela declara sobre o que apurou** e diz o que **não** verificou: mês usado, tamanho do catálogo, corte aplicado, e a ressalva do código de barras sempre visível. O corte padrão é **200 unidades no bairro**, calibrado em junho/2026 (sem corte a regra devolve **44.874** avisos; com 200, sobram 156 avisos e 116 produtos). O rótulo diz "no bairro" e não "por loja" porque o contador de PDVs concorrentes não chegou ao banco.
  - **A lista não estima demanda**, e a tela diz isso: ela informa quanto o bairro move, não quanto a rede venderia — e não há como prever a venda de um produto que a rede nunca vendeu.
  - **Falta**: o extrator declarar `ProdutosAtivosSemEan` no manifesto, para a ressalva da tela trocar a frase medida na Natusfarma pelo número real da rede que está olhando.
- [x] **F17 — Excesso de zeros: o experimento do objetivo de treino**: o patrocinador aprovou testar "trocar a régua de aprendizado do modelo" depois de a comparação mostrar o ML perdendo em todas as faixas de demanda positiva. A resposta de manual — trocar a função de perda para Tweedie ou Poisson — **não é alcançável no ML.NET** (ver §2): o `objective` do LightGBM não é exposto. O que cabe é o **modelo em dois estágios** (`HurdleForecastEngine`): um LightGBM binário estima `P(vende no dia)`, um segundo LightGBM ajustado **só nos dias com venda** estima `E[unidades | vendeu]`, e a previsão é o produto. Mesmas features de F5, nenhum pacote novo, plugado em `IForecastEngine` — que existe desde a F6 exatamente para isto.
  - **Medido em dado real da rede** (Natusfarma, 2025-07-29 a 2026-06-14, walk-forward de 4 janelas × 28 dias), em três recortes de catálogo, para separar o efeito da esparsidade do efeito do modelo:

    | recorte | dias-item sem venda | WAPE L2 | WAPE hurdle | MAE L2 | MAE hurdle | viés L2 | viés hurdle |
    |---|---|---|---|---|---|---|---|
    | top 1.200 SKUs | 66,7% | 1,1870 | **1,1674** | 3,8846 | **3,8205** | −1,3% | +3,2% |
    | top 4.000 SKUs | 74,5% | 1,2436 | **1,2162** | 1,8422 | **1,8017** | −4,6% | +1,0% |
    | catálogo inteiro (11,4 mil SKUs) | 86,6% | 1,2988 | **1,2654** | 0,7661 | **0,7464** | −7,7% | **−2,2%** |

  - **O viés do regressor único cresce monotonicamente com a esparsidade** — −1,3%, −4,6%, −7,7% — e é a confirmação em dado real do diagnóstico que abriu a fase: não é intermitência, é a média condicional sendo puxada para baixo pelos dias sem venda. No catálogo inteiro, que é o ponto de operação de verdade, o hurdle ganha em **WAPE, MAE e RMSE** (4,8987 contra 4,9269) e **reduz o viés a menos de um terço**. Duas ressalvas honestas: o MAPE fica ligeiramente **pior** (0,695 contra 0,690) — é a métrica que menos vale aqui, por quebrar em demanda baixa (CLAUDE.md §6) —, e **o ganho de viés é específico da série esparsa**: no recorte dos 1.200 SKUs de maior volume o regressor único já é quase não-enviesado (−1,3%) e o hurdle passa do alvo (+3,2%). O hurdle não é uniformemente melhor; ele é melhor onde o excesso de zeros morde, que por acaso é onde o catálogo de verdade vive.
  - **Na classe B o LightGBM simples é pior que o baseline naïve** (WAPE 1,5197 contra 1,4870, recorte de 4 mil SKUs) e o hurdle bate os dois (1,4552). É regressão local que a média global escondia — exatamente o que §6 manda procurar.
  - **O engine salvo continua sendo o LightGBM simples, de propósito**, e o resultado do treino passou a declarar `ModeloSalvo` ao lado de `MelhorEngine`. Menor WAPE é critério de erro por unidade e **não** de decisão de compra: um engine que encolhe previsões na direção do zero melhora o WAPE numa série com 86,6% de dias sem venda e compra menos do que a loja precisa. A tela de `/treinamento` mostrava um troféu no vencedor do backtest — que já podia ser o naïve — como se fosse declaração de produção; agora mostra os dois selos.
  - **Achado de correção, e não de modelagem: a ordem do clamp em zero.** O `WalkForwardBacktest` sempre truncou a previsão em zero **por dia**; os dois comparadores contra o ERP truncavam **depois de tirar a média da janela**, então um dia com previsão negativa abatia um dia positivo e a taxa de demanda do braço ML caía abaixo do que qualquer dia isolado justificava — o mesmo modelo pontuado por duas contas. O regressor por erro quadrático produz negativo justamente onde a série é esparsa: **12,5% dos dias** no recorte de 1.200 SKUs. O erro só existia na direção de comprar menos. Corrigido nos dois comparadores, com teste que falha na aritmética antiga. **Consequência para a leitura dos resultados:** sessão materializada antes desta correção guarda o número da conta antiga, e o resultado é gravado, nunca recalculado (ver §3) — então comparação entre sessões anteriores e posteriores à correção não é estritamente comparável no braço ML. A diferença medida no recorte real fica entre 0,2% e 1,1% do volume do braço, sempre para cima.
  - **Onde o dado sintético engana, e por que o experimento não parou nele.** Num painel sintético empurrado a 96% de zeros o hurdle subestimava o volume em 73% e parecia perigoso; em dado real a 86,6% ele é o menos enviesado dos quatro. A esparsidade artificial foi obtida derrubando a probabilidade de venda de todos os perfis, o que remove o sinal junto com as vendas — o dado real é esparso **e** estruturado. Vale como aviso metodológico: painel sintético serve para provar mecanismo, nunca para calibrar decisão.
  - **Ressalva de leitura: WAPE acima de 1,0 em todos os quatro engines.** Em série diária com 86,6% de dias sem venda o erro absoluto supera a demanda total, e nenhum destes modelos é bom em termos absolutos nessa granularidade. A comparação entre eles é válida — mesma população, mesmas janelas —, a leitura de "quão bom" não é.
  - **Decidido em 2026-09-09: o hurdle NÃO vira engine de produção.** A decisão dependia de medir a **decisão de compra**, e não o erro por unidade — foi medida, com `PurchasingSimulator` replicando a reposição dia a dia sobre o mesmo histórico e a mesma política (`SimulationOptions` default: lead time 7, ciclo 7, fator de serviço 1,65) para os dois engines:

    | KPI | LightGBM | hurdle | leitura |
    |---|---|---|---|
    | venda perdida (un.) | 63.236 | **61.650** | hurdle, −2,5% |
    | dias com ruptura | **4.902** | 5.159 | LightGBM |
    | nível de serviço un. | 78,65% | **79,18%** | hurdle, +0,53 p.p. |
    | nível de serviço dias | **93,30%** | 92,95% | LightGBM |
    | unidades pedidas | **287.712** | 291.207 | hurdle compra +1,2% |
    | estoque médio | **49** | 50 | empate |

    **É empate com trocas, não vitória:** o hurdle perde menos venda no total e protege pior a cauda — mais dias de prateleira vazia —, e paga o ganho comprando 1,2% mais. Meio ponto de nível de serviço em unidades nessas condições não justifica trocar o engine que serve as sugestões. **O custo total do simulador (99.372 contra 98.022, −1,4% para o hurdle) foi deliberadamente EXCLUÍDO da decisão**: ele é função direta de `CustoCarregamentoDia` e `CustoRupturaUnidade`, dois pesos arbitrados no default e não medidos na operação — com outros pesos o sinal inverte. Se algum dia vierem da rede, a conta merece ser refeita. Os níveis de serviço absolutos também refletem os parâmetros da política e o estoque inicial escolhido no harness, não a operação da Natusfarma.
  - **O que o experimento entrega mesmo com resposta negativa**: a medição do viés por esparsidade em dado real, a constatação de que Tweedie/Poisson não é alcançável no ML.NET (resultado, não obstáculo), e a demonstração de que **menor WAPE não é melhor decisão de compra** — que é o que sustenta a separação `MelhorEngine` / `ModeloSalvo`. O hurdle segue rodando em todo treino e aparecendo na comparação walk-forward, então o número chega sozinho a cada execução, sem custo. **Isto não desbloqueia a F9**, que continua travada no horizonte de 7 dias: esta medição compara dois engines entre si sobre a mesma política, e não a política forecast-based contra o ERP de ciclo completo.
  - **`LightGbmForecaster` passou a aceitar qualquer `IForecastModel`** (era o tipo concreto do LightGBM de um estágio), que é o que tornou esta medição possível. O adaptador nunca precisou de mais que `Predict(FeatureVector)`.
