...
### Pacotes preferidos
- **ML.NET:** `Microsoft.ML`, `Microsoft.ML.TimeSeries`, `Microsoft.ML.LightGbm`, `Microsoft.ML.FastTree`, `Microsoft.ML.AutoML`. **O LightGBM tem dependência nativa que não é resolvida por NuGet:** `lib_lightgbm.so` declara `libgomp.so.1`, e nenhuma imagem oficial do .NET traz esse pacote. Por isso o `worker` — único serviço que treina — sai de uma base própria (`worker-base.Dockerfile`, `ContainerBaseImage` no `.csproj`), não da base default do container publishing do SDK. Isso não aparece no `F5`, porque no Windows o nativo usa o OpenMP da MSVC; aparecia só no treino, em container, como `Unable to load shared library 'lib_lightgbm'`. Ao trocar a base ou o major do runtime, mantenha a base e o smoke do CI que roda `ldd` no nativo — os testes de treino passam num runner que tem `libgomp1` e **não** cobrem a imagem (ver README §5).
...
|---|---|---|---|
| `Stage` | SQL Server | **DACPAC** — SQL Server Project `CosmosPro.ML.DemandForCast.Database`, publicado pelo `db-migrator` via `Microsoft.SqlServer.DacFx` (com `DropObjectsNotInSource` desde a F16 — tabela que sai do source sai do banco). | Staging area dos dados importados pelo usuário via UI (CSV/ZIP). Tabelas plural: `Redes`, `Lojas` (com `Cnpj` desde a F16), `Produtos`, `Vendas`, `EstoquesDiarios`, `Compras`, `Promocoes`, `SinaisExternos`, `SugestoesCompra`, `SugestoesCompraItens`. `MercadoIqvia` **saiu na F16** — o dado de mercado tem ciclo de vida oposto ao do Stage (ver abaixo). Engine só lê, nunca escreve — o **Worker** é o único que escreve. |
| `engine` | SQL Server | **EF Core migrations**, aplicadas pelo `db-migrator` com `Database.MigrateAsync()`. Source-of-truth no projeto `CosmosPro.ML.DemandForCast.Engine`. | Metadados do engine: `Redes` (registro dos inquilinos, fonte de verdade do `RedeId`), `CargasStage`, `TreinoJobs`, `SimulacoesCompra`, `ComparacoesPbs`, **`ComparacaoSessoes`** e **`ComparacaoSessaoItens`** (F14), **`Questionarios`** e **`QuestionarioRespostas`** (F15), **`MercadoCargas`**, **`MercadoObservacoes`**, **`MercadoProdutos`** e **`MercadoBrickPdvs`** (F16, dados de mercado IQVIA) — todas ver abaixo —, e as 7 tabelas do ASP.NET Core Identity (`AspNetUsers` com `RedeId` + FK para `Redes`, `AspNetRoles`, etc). |
...
**Dados de mercado IQVIA (F16): ciclo de vida oposto ao do Stage, e é por isso que vivem no `engine`.** O Stage significa "substituído inteiro a cada import"; o dado de mercado é da rede, sobrevive aos imports e é reaproveitado por várias comparações. O comprador envia o XLSX mensal da IQVIA na tela **Dados de mercado** (`/mercado`), fila própria (`MercadoCargas` → bucket `mercado` no MinIO → `MercadoWorker`/`MercadoProcessor`), mesma fila-pattern das demais. Regras que não são estilo:

- **Recarga substitui por `(RedeId, Mes, Brick)`**, nunca por rede: cada arquivo traz o mês e o espelho do ano anterior, e a série mensal só existe empilhando N arquivos — um `DELETE ... WHERE RedeId` destruiria a série; nenhum DELETE duplicaria PK no reenvio.
- **Zero ≠ não-coberto.** Só células com medida ≠ 0 viram linha em `MercadoObservacoes` (~72% do arquivo real é zero). Dentro de um `(mês, brick)` coberto, ausência de linha é venda zero; fora, é "nunca enviado". Quem separa os dois é a cobertura (`GET /api/mercado/cobertura`, agregada do dado) e o `ResumoJson` da carga — a cobertura vem das **colunas presentes**, não do filtro da consulta IQVIA (o arquivo real pede bricks no filtro que não geram coluna nenhuma).
- **`ValorCpp` é valor ao consumidor sob a metodologia da IQVIA, que NORMALIZA preços entre players.** Não é preço de balcão nem preço de aquisição — não comparar com `SugestoesCompraItens.PrecoCompra`, e `ValorCpp/Unidades` é preço-índice, não preço praticado.
- **O cabeçalho de medida é contrato** (`Brick rpe <brick> Bandeira <bandeira> Mes <AAAAMM> Unidades|Real CPP`): coluna que não casa é **falha legível com o texto do cabeçalho**, não coluna pulada — ignorar perderia um recorte inteiro em silêncio. O parser (`IqviaXlsxParser`) é streaming (`XmlReader`, sem DOM — 60 MB de XML) e **soma EANs duplicados** no mesmo recorte (o arquivo real tem 52.837 linhas para 52.804 EANs; sem a soma, o PK estoura no bulk).
- **A ponte com as lojas é CNPJ.** A aba de PDVs dá brick → CNPJs do painel (`MercadoBrickPdvs`; `00000000000000` é o agregado anônimo de concorrentes), e `Stage.Lojas.Cnpj` (novo na F16, extraído de `ENTIDADES` no PBS) é o outro lado do join — feito por valor, na leitura, na parte C (features de mercado, ainda não implementada). A aba usa `_` no brick onde a de dados usa espaço; o parser normaliza.
- O escopo por rede segue o padrão: `RedeId` **nunca vem do arquivo**, endpoints respondem **404 (não 403)** para carga de outra rede, e o `redeId` na Web sai do `IRedeContext`.

**Alertas de mercado no item da sessão (F16 parte C, grupo B): as sete colunas de `Mercado*` seguem o mesmo contrato de nulo do braço de ML, e por um motivo mais forte.** Nulo é "não foi possível calcular"; **zero é medição da IQVIA**, e zero na coluna da rede com concorrentes vendendo é o alerta mais forte que a tela produz — o item está no cadastro, está na sugestão, o bairro vende e a rede vendeu nada. Colapsar um no outro inverte a frase. Cinco causas legítimas de nulo: loja sem `Cnpj`, CNPJ fora do painel, SKU sem `Ean`, EAN que a IQVIA não reportou, e nenhum mês coberto antes do mês da sugestão. O alerta tem **cinco** estados e o quinto não é enfeite: `null` (não avaliado), `SemAlerta`, `Ruptura`, `SemCausa` e `NaoApurado` — este último cobre participação baixa com o mês comparado fora do histórico de estoque, e mandá-lo para `SemCausa` faria o software afirmar que não houve ruptura sem ter olhado.

- **A régua do B2 é relativa à própria rede, não fatia de mercado.** `fatia do item no brick ÷ fatia agregada da rede no mesmo brick`. A régua por número de lojas **não roda**: o contador de PDVs concorrentes vive numa área de tabela dinâmica que `IqviaXlsxParser.LerAbaDePdvs` ignora de propósito, então `MercadoBrickPdvs` tem **uma** linha de concorrentes (CNPJ zerado), não 37. Se o contador algum dia entrar, a régua muda e o número publicado (mediana 1,57, dispara em 8,5%) passa a valer; a régua atual dá mediana 1,13 e dispara em 15,0%.
- **O mês comparado é sempre estritamente anterior ao mês da sugestão** (`MercadoMesResolver`). Não é purismo: o mês da sugestão contém as consequências dela, e comparar contra ele torna circular a afirmação "o alerta teria avisado o comprador", que é o resultado que a dissertação sustenta. Com um relatório só carregado a regra cai no espelho do ano anterior, que todo arquivo traz.
- **EAN do PBS tem 14 caracteres com zero à esquerda; o da IQVIA tem 13. Comparação exata casa ZERO.** Medido em 2026-08-30 contra o cadastro real. `MercadoSinalLoader.NormalizarEan` tira os zeros dos dois lados. Ao mexer em qualquer join por EAN, lembre que a falha aqui é **silenciosa**: dicionário vazio, nenhuma exceção, nenhum log, e a tela mostra travessão em toda linha como se a rede fosse bem.
- **EAN que a IQVIA não reportou não é falha de join.** A lista de EANs do relatório é *o que teve movimento nos bricks pedidos*, não um catálogo. Ausência num recorte coberto significa que o mercado daquele bairro não vendeu o item — informação, e não erro.
- **O filtro "só com alerta" lista os três alertas de verdade, nunca nega `SemAlerta`.** Nulo não sobrevive a comparação de desigualdade em SQL, e item sem dado de mercado não é alerta — é "não avaliado". O filtro entra em `ComparacoesEndpoints.AplicarFiltros`, a cláusula única por onde página, contagem, totalizadores e Excel já passam.
- **Duas armadilhas de teste encontradas aqui, ambas invisíveis ao compilador.** (1) **Componente Radzen não repassa atributo desconhecido ao DOM** — `data-test` num `RadzenCheckBox` não existe no HTML, e o E2E falha por timeout sem dizer por quê; ponha o atributo num elemento HTML puro. (2) **O `INSERT` da fixture E2E (`AppHostFixture.SemearSessaoConcluidaAsync`) lista as colunas à mão** — coluna nova em `ComparacaoSessaoItens` precisa entrar lá também, senão o teste afirma "sem dado de mercado" sem que nada no caminho da tela esteja errado.

**Catálogo de códigos de barras da rede (F16 parte C, grupo A): `engine.RedeCatalogoEans`, e a razão de não ser tabela do Stage é a mesma que tirou a `MercadoIqvia` de lá.** A tela `/mercado/oportunidades` não pertence a sessão nenhuma, e cada import apaga o Stage da rede — no Stage o catálogo zeraria no envio seguinte e a tela passaria a oferecer **o cadastro inteiro** como oportunidade de sortimento. É o único CSV do ZIP sem tabela no Stage, e há teste em `StageContractTests` afirmando essa ausência para ela não ser "corrigida".

- **Substituição por `RedeId` inteiro** a cada envio, e não por chave parcial: é **retrato** do cadastro, não série histórica. Produto descadastrado tem de sair, senão a tela deixa de oferecer algo que a rede já teve. O oposto de `MercadoObservacoes`, cuja recarga é por `(mês, brick)` justamente para preservar a série que vários arquivos empilham.
- **ZIP sem `catalogo_eans.csv` não apaga o catálogo que existia.** Extrator anterior à 0.18.0 não o traz; apagar puniria o comprador por usar um build velho, tirando dele uma tela que funcionava.
- **Catálogo vazio devolve lista vazia, nunca o mercado inteiro.** Zero códigos significa "o arquivo não foi enviado", e não "a rede não tem produto nenhum" — devolver tudo seria a tela errando em 100% das linhas. A tela explica em vez de listar (`data-test="aviso-sem-catalogo"`).
- **Só a bandeira `CONCORRENTES` conta na oportunidade.** Somar a bandeira própria faria a tela oferecer o próprio sortimento de volta, e infla o corte de relevância com venda da própria rede.
- **`Engine.Mercado.Ean.Normalizar` é a única regra de normalização, e os três lados do join dependem dela** — quem grava o catálogo (import), quem grava as observações (parser) e quem cruza os dois. Não duplicar: o PBS grava 14 caracteres com zero à esquerda e a IQVIA grava 13, então comparação exata casa **zero** e a falha é silenciosa.
- **O corte de relevância (A2) é em unidades absolutas do bairro, não por loja concorrente**, porque o contador de PDVs do painel não chegou ao banco. Padrão de **200**, calibrado em junho/2026 — sem corte a regra A1 devolve 44.874 avisos. A distorção conhecida: favorece o brick com mais lojas concorrentes.
- **Armadilha de E2E: o teste entra como PowerUser, que não tem rede vinculada e cai na primeira rede ativa por id.** Semear numa rede criada pelo próprio teste deixa a tela olhando outro inquilino, e ela renderiza "sem catálogo" com toda razão — 60s de timeout sem dizer o motivo. Semeie na primeira rede ativa, como os demais semeadores da fixture.
